...
public class DeleteUserInGroupCommandTests
{
    ...
    public DeleteUserInGroupCommandTests()
    {
    ...
    [Fact]
    public async Task Should_Delete_UserInGroup()
    {
        // Arrange
        var deleteUserInGroupCommand = new DeleteUserInGroupCommand(_mockUserInGroupId);

        // Act
        await _userInGroupCommandHandler.Handle(deleteUserInGroupCommand, CancellationToken.None);
        ...
        _bus.Verify(x => x.Publish(It.Is<UserInGroupDeletedEvent>(g => g.Id == _mockUserInGroupId), CancellationToken.None));
    }

    [Fact]
    public async Task Should_Not_Delete_UserInGroup_Invalid_Empty_Id()
    {
        // Arrange
        var deleteUserInGroupCommand = new DeleteUserInGroupCommand(Guid.Empty);
        ...
        _bus.Verify(x => x.Publish(It.Is<UserInGroupDeletedEvent>(g => g.Id == _mockUserInGroupId), CancellationToken.None), Times.Never);
    ...
    [Fact]
    public async Task Should_Not_Delete_UserInGroup_Not_Exist()
    {
        // Arrange
        var deleteUserInGroupCommand = new DeleteUserInGroupCommand(_mockNonExistingUserInGroupId);

        // Act
        await _userInGroupCommandHandler.Handle(deleteUserInGroupCommand, CancellationToken.None);

        // Assert
        _userInGroupRepository.Verify(x => x.RemoveAsync(It.Is<Guid>(g => g == _mockNonExistingUserInGroupId)), Times.Never);

        _uow.Verify(x => x.CommitAsync(), Times.Never);

        _bus.Verify(x => x.Publish(It.Is<UserInGroupDeletedEvent>(g => g.Id == _mockNonExistingUserInGroupId), CancellationToken.None), Times.Never);
